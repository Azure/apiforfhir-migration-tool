...
namespace FhirMigrationTool.Security
{
    ...
    public class BearerTokenHandler : DelegatingHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            if (request.RequestUri is not null && request.RequestUri.Scheme != Uri.UriSchemeHttps && request.RequestUri.Host != "localhost")
            {
                throw new InvalidOperationException("Bearer token authentication is not permitted for non TLS protected (https) endpoints.");
            }

            var scopes = _scopes;
            if (scopes is null or { Length: 0 } && request.RequestUri is not null)
            {
